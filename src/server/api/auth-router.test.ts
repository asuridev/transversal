...
test('auth-router', async (t) => {
  ...
  await t.test('P2: GET /auth/callback emite bo_session+csrf y descarta el token del IdP', async () => {
    await withServer(baseDeps(), async (baseUrl) => {
  ...
  await t.test('P9: callback de asesor con partner INEXISTENTE ⇒ 302 /forbidden sin sesión (FR-008)', async () => {
    await withServer(
      baseDeps({
        exchangeAuthorizationCode: async () => ({ sub: 'u-x', name: 'X', partner: 'banco-fantasma' }),
        partnerRepository: stubPartnerRepository({}),
      }),
      async (baseUrl) => {
        const loginRes = await fetch(`${baseUrl}/auth/login`, { redirect: 'manual' });
  ...
  await t.test('P10: callback de asesor con partner INACTIVO ⇒ 302 /forbidden sin sesión (FR-008)', async () => {
    await withServer(
      baseDeps({
        exchangeAuthorizationCode: async () => ({ sub: 'u-y', name: 'Y', partner: 'banco-inactivo' }),
        partnerRepository: stubPartnerRepository({
  ...
  await t.test('CT-07: ningún token del IdP aparece en cookies ni cuerpo de la respuesta (SC-003)', async () => {
    await withServer(
      baseDeps({
        exchangeAuthorizationCode: async () => ({
          sub: 'u-4',
          ...
          id_token: 'should-not-leak-either',
        }),
      }),
      async (baseUrl) => {
        const loginRes = await fetch(`${baseUrl}/auth/login`, { redirect: 'manual' });
  ...
  await t.test('CT-11: POST /auth/logout expira cookies y produce URL de end_session_endpoint (FR-014)', async () => {
    await withServer(
      baseDeps({
        postLogoutRedirectUri: 'https://webview-login.example.com',
        endSession: async ({ postLogoutRedirectUri }) =>
          new URL(
            `https://idp.example.com/realms/backoffice/protocol/openid-connect/logout?post_logout_redirect_uri=${encodeURIComponent(postLogoutRedirectUri)}`,
          ),
      }),
      async (baseUrl) => {
        const token = issueCsrfToken();
