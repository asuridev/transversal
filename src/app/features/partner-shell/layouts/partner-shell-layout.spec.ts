...
import { PartnerShellLayout } from './partner-shell-layout';

describe('PartnerShellLayout — cerrar sesión', () => {
  ...
  beforeEach(() => {
    ...
    // El botón de logout solo se renderiza para una sesión autenticada.
    store.setUser({ subject: 'u-1', name: 'Ana Asesora', roles: [] });
  });

  afterEach(() => httpMock.verify());

  function create() {
    const fixture = TestBed.createComponent(PartnerShellLayout);
  ...
  it('sin sesión: no renderiza el botón de cerrar sesión', () => {
    store.clear();
    const { fixture } = create();
    const button = fixture.nativeElement.querySelector('button[aria-label="Cerrar sesión"]');
